...
const convertCurrency = async (selectedCurrency: string, value: number) => {
...
const Convertor = async () => {
  console.log(
    chalk.blueBright(
      "|||||||||||||||||||||||||||||| WELCOME TO OUR CURRENCY CONVERTER PAGE ||||||||||||||||||||||||||||||"
    )
  );
  ...
  const currencies = Object.keys(json).filter(
    (currency) =>
      currency.startsWith("usd") ||
  ...
  if (currencies.includes(selectedCurrency)) {
    console.log(
      chalk.greenBright(`Selected currency "${selectedCurrency}" is valid.`)
    );
    ...
    console.log(
      chalk.redBright(`Selected currency "${selectedCurrency}" is not valid.`)
    );
  }
};
